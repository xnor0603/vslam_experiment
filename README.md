...
四個檔案三件事：(1) 控飛機飛 5m、(2) 修補 cuVSLAM 的 TF 鏈、(3) 後處理畫軌跡。
...
|---|---|---|
| `forward_5m_experiment.py` | 起飛指令節點：起飛 → 懸停 → 前進 5m → 懸停 → 降落 | 任意位置可跑；要 `ros2 run` 就丟進某個 ROS 2 Python pkg |
...
| `sitl_mode` | False | True 時切 TRANSIENT_LOCAL QoS（給 SITL bridge 用） |
...
## 2. cuVSLAM 端 — `isaac_ros_visual_slam_bag.launch.py`
...
## 3. 分析端 — `plot_three_trajectories.py` / `plot_single_trajectory.py`
...
- 圖檔：`./figures/<bag起始時間>_trajectories.png`

> 注意：腳本內 `DEFAULT_TRIM` / `DEFAULT_VSLAM` 寫死了在原機器的舊 bag 路徑，新機器上**一定要傳 CLI 參數**，或改 default。
...
兩支腳本都寫死輸出到 `/home/landis/VSLAM/ClaudeCode/figures/` — 在新機器上**先改路徑**，不然會找不到資料夾。
...
| 步驟 | 動作 | 證據 |
|---|---|---|
| 1 | 地面用粉筆/膠帶標起點 A，皮尺往機頭方向拉 5m 標 B 點 | 拍照 |
