...
PRESETS = {
    ...
    #
    # 重要：SIM_GPS_NOISE_H 是「per-sample 白噪 σ」，不是 CEP / EPH。
    # 真 M8N 在開闊地 EPH 約 2 m，但那來自慢漂 bias，單次 sample 白噪只有 ~0.4 m。
    # 經驗法則：σ ≲ EPH/2，才能讓 EKF 殘差落在信任區間內、preflight 不會 fail。
    'm8n_open': {
        'desc': 'UBLOX M8N 開闊天空 (典型 outdoor)',
        'SIM_GPS_NOISE_H': 0.4, 'SIM_GPS_NOISE_V': 0.7,
    ...
    'm8n_suburban': {
        ...
        'SIM_GPS_NOISE_H': 1.0, 'SIM_GPS_NOISE_V': 1.5,
    ...
    'tree_cover': {
        ...
        'SIM_GPS_NOISE_H': 2.0, 'SIM_GPS_NOISE_V': 3.0,
